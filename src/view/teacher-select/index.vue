<template>
  <a-spin :spinning="loading">
    ...
    <no-select :description="curStageInfo.description" v-if="curStageInfo.stage === -999"></no-select>
    <Course v-else />
  </a-spin>
</template>
...
const init = async () => {
  loading.value = true
  try {
    await termStore.setTeacherTermInfo()
    console.log('termInfo;;', termInfo.value);
    console.log(curStageInfo);
    loading.value = false
  } catch (e) {
    notification.error({ message: '双向选课', description: '获取课程数据异常'+e.message })
    loading.value = false
  }
}
init()
