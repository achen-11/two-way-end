 ...
 * 
 */
export const getStuSelectedNum = (selections: Selection[], term: Term, stage: number) => {
  let num = 0
  selections.forEach(record => {
    if (record.term_id === term.id) {
      ...
      if (stage === 1) {
        // 第一阶段 
        if (record.stage === 1) num++
      } else if (stage === 2) {
        // 第二轮 = 第一轮 agree + 第二轮 all
        if (record.stage === 1 && record.status === 1) num++
        if (record.stage === 2) num++
      } else {
        // 第三轮 = 第一轮 agree + 第二轮 agree + 第三轮 all
        if (record.stage === 1 && record.status === 1) num++
        if (record.stage === 2 && record.status === 1) num++
        if (record.stage === 3) num++
      }
    }
  })
  return num
