 ...
 * 校验班级数据
 */
export const validateClass = (classInfo, allMajor: Major[]) => {
  const { name, major_name, enroll_year } = classInfo
  const errorInfo: string[] = []
  // 校验名称
  if (!name) errorInfo.push('班级名为空')
  // 校验专业
  if (!major_name) errorInfo.push('专业为空')
  if (!allMajor.some(item => item.name === major_name)) errorInfo.push('专业不存在, 请前往专业管理模块添加')

  // 入学年份
  if (!enroll_year) errorInfo.push('入学年份为空')
  if (isNaN(Number(enroll_year)) || +enroll_year < 2000 || +enroll_year > 2300) errorInfo.push('非法入学年份')
